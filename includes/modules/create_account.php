...
if (isset($_POST['action']) && ($_POST['action'] == 'process') && !isset($login_page)) {
    ...
    if ($error == true) {
        ...
        if ($send_welcome_email) {
            ...
            $block = [];
            if (ACCOUNT_GENDER == 'true') {
                $block['EMAIL_GREETING'] = sprintf($gender == 'm' ? EMAIL_GREET_MR : EMAIL_GREET_MS, $lastname);
            } else {
                $block['EMAIL_GREETING'] = sprintf(EMAIL_GREET_NONE, $firstname);
            }
            $block['EMAIL_FIRST_NAME'] = $firstname;
            $block['EMAIL_LAST_NAME'] = $lastname;

            // initial welcome
            $block['EMAIL_WELCOME'] = EMAIL_WELCOME . $extra_welcome_text;
            ...
            if (NEW_SIGNUP_DISCOUNT_COUPON != '' and NEW_SIGNUP_DISCOUNT_COUPON != '0') {
                ...
                $block['COUPON_TEXT_VOUCHER_IS'] = EMAIL_COUPON_INCENTIVE_HEADER;
                $block['COUPON_DESCRIPTION'] = (!empty($coupon_desc->fields['coupon_description']) ? '<strong>' . $coupon_desc->fields['coupon_description'] . '</strong>' : '');
                $block['COUPON_TEXT_TO_REDEEM'] = str_replace("\n", '', sprintf(EMAIL_COUPON_REDEEM, ''));
                $block['COUPON_CODE'] = $coupon->fields['coupon_code'];
            ...
            if (NEW_SIGNUP_GIFT_VOUCHER_AMOUNT > 0) {
                ...
                if (SEARCH_ENGINE_FRIENDLY_URLS == 'true') {
                    $url = HTTPS_SERVER . DIR_WS_CATALOG . 'index.php/gv_redeem/gv_no/';
                } else {
                    $url = zen_href_link(FILENAME_GV_REDEEM, 'gv_no=' . $coupon_code, 'NONSSL', false);
                }

                $block['GV_WORTH'] = str_replace('\n', '', sprintf(EMAIL_GV_INCENTIVE_HEADER, $currencies->format(NEW_SIGNUP_GIFT_VOUCHER_AMOUNT)));
                $block['GV_REDEEM'] = sprintf(EMAIL_GV_REDEEM, '<strong>' . $coupon_code . '</strong>');
                $block['GV_LINK_TEXT'] = EMAIL_GV_LINK . $url;
                $block['GV_LINK_HTML'] = EMAIL_GV_LINK . '<a href="' . zen_href_link(FILENAME_GV_REDEEM, 'gv_no=' . $coupon_code, 'NONSSL', false) . '">' . TEXT_GV_NAME . ': ' . $coupon_code . '</a>';
                $block['GV_LINK_OTHER'] = EMAIL_GV_LINK_OTHER;
            } // endif voucher

            // add in regular email welcome text
            $block['EMAIL_MESSAGE'] = EMAIL_TEXT;
            $block['EMAIL_CONTACT_OWNER'] = str_replace('\n', '', EMAIL_CONTACT);
            $block['EMAIL_CLOSURE'] = EMAIL_CLOSURE;

            // include create-account-specific disclaimer
            $block['EMAIL_DISCLAIMER'] = sprintf(EMAIL_DISCLAIMER_NEW_CUSTOMER, '<a href="mailto:' . STORE_OWNER_EMAIL_ADDRESS . '">' . STORE_OWNER_EMAIL_ADDRESS . ' </a>');
